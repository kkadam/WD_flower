...
!         IF (ppoly(J,K,L).LT.0.0d0) ppoly(J,K,L) = 0.0
!         ppoly(J,K,L) = p(J,K,L) - pdeg(J,K,L)
!         IF (ppoly(J,K,L).LT.0.0d0) ppoly(J,K,L) = 0.0
         IF (rho(J,K,L).GT.p_den_cutoff) THEN
          c(J,K,L) = sqrt(gamma*ppoly(J,K,L)/rho(J,K,L)+eight_a_b_hat/3.0*x_sq/SQRT(1.0+x_sq))
         ELSE
          c(J,K,L) = sqrt(eight_a_b_hat/3.0*x_sq/SQRT(1.0+x_sq))
         ENDIF
      enddo
   enddo
enddo
